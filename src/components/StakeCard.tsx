...
function StakeCard(props:any){
    ...
    async function approve(){
        ...
        if (value){
          console.log( Web3.utils.toWei(String(value),'ether'));
            c.methods.approve(pooladd,Web3.utils.toWei(String(value),'ether')).send({from:account}).on('transactionHash',(hash:any) => {
    ...
    async function Deposit(){
        ...
        if (value){
            c.methods.deposit(Web3.utils.toWei(String(value),'ether'),getRange(),account).send({from:account}).on('transactionHash',(hash:any) => {
                addToast("Transaction Created : "+hash, {
                    appearance: 'success',
                    autoDismiss: true,
                  })
